...
from tests.utils import ThrottleT, requires_throttle_type
...
class MockConnection:
    """Mock connection with a scope dict for rule testing."""
...
class TestThrottleRuleInit:
    ...
    def test_predicate_stored(self) -> None:
        ...
        rule = ThrottleRule(predicate=predicate)
    ...
    def test_predicate_context_detection(self) -> None:
        ...
        rule1 = ThrottleRule(predicate=predicate_no_ctx)
        assert rule1._predicate_takes_context is False

        rule2 = ThrottleRule(predicate=predicate_with_ctx)
...
class TestThrottleRuleCheck:
    async def test_no_constraints_matches_everything(self) -> None:
        rule = ThrottleRule()
        assert await rule.check(MockConnection(method="GET", path="/anything")) is True

    async def test_method_match(self) -> None:
        rule = ThrottleRule(methods={"GET"})
        assert await rule.check(MockConnection(method="GET", path="/")) is True
        assert await rule.check(MockConnection(method="POST", path="/")) is False
    ...
    async def test_path_match(self) -> None:
        rule = ThrottleRule(path="/api/")
        assert await rule.check(MockConnection(path="/api/users")) is True
        assert await rule.check(MockConnection(path="/public/")) is False

    async def test_path_regex(self) -> None:
        rule = ThrottleRule(path=re.compile(r"/api/v\d+/"))
        assert await rule.check(MockConnection(path="/api/v1/users")) is True
        assert await rule.check(MockConnection(path="/api/vX/users")) is False

    async def test_path_and_method_combined(self) -> None:
        rule = ThrottleRule(path="/api/", methods={"POST"})
        assert await rule.check(MockConnection(method="POST", path="/api/data")) is True
        assert await rule.check(MockConnection(method="GET", path="/api/data")) is False
        assert await rule.check(MockConnection(method="POST", path="/public/")) is False
    ...
    async def test_predicate_runs_after_path_and_method(self) -> None:
        ...
        await rule.check(MockConnection(method="POST", path="/api/data"))
        assert call_count == 0

        # Path mismatch -> predicate never called
        await rule.check(MockConnection(method="GET", path="/public/"))
        assert call_count == 0

        # Both match -> predicate called
        await rule.check(MockConnection(method="GET", path="/api/data"))
        assert call_count == 1
...
class TestBypassThrottleRuleCheck:
    ...
    async def test_no_constraints_always_bypasses(self) -> None:
        ...
        assert await rule.check(MockConnection(method="GET", path="/anything")) is False

    async def test_method_match_bypasses(self) -> None:
        rule = BypassThrottleRule(methods={"GET"})
        # GET matches -> bypass (False)
        assert await rule.check(MockConnection(method="GET", path="/")) is False
        # POST doesn't match -> don't bypass (True)
        assert await rule.check(MockConnection(method="POST", path="/")) is True

    async def test_path_match_bypasses(self) -> None:
        rule = BypassThrottleRule(path="/api/")
        assert await rule.check(MockConnection(path="/api/users")) is False
        assert await rule.check(MockConnection(path="/public/")) is True
    ...
    async def test_path_and_method_combined(self) -> None:
        ...
        assert await rule.check(MockConnection(method="GET", path="/public/")) is True
