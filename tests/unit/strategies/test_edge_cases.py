...
class TestRateEdgeCases:
    ...
    async def test_very_small_rate_limit(
    ...
    async def test_very_large_rate_limit(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
    ...
    async def test_very_long_time_window(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
    ...
    async def test_fractional_rates(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
...
class TestConcurrencyEdgeCases:
    ...
    async def test_extreme_concurrency(
        ...
        wait = await strategy(key, rate, backend, cost=50)
        assert wait > 0, "101st request should be throttled"

    async def test_race_condition_at_boundary(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
...
class TestTimingEdgeCases:
    ...
    async def test_requests_across_multiple_windows(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
...
class TestStrategyStateEdgeCases:
    ...
    async def test_strategy_with_corrupted_state(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
    ...
    async def test_backend_connection_during_request(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
...
class TestWaitTimeCalculations:
    ...
    async def test_wait_ms_never_negative(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
    ...
    async def test_wait_ms_reasonable_bounds(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
