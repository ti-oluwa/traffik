...
- **Enhancements**:
  ...
  - Added `traffik.config.get_legacy_md5_keys()`/`set_legacy_md5_keys()` (env var `TRAFFIK_LEGACY_MD5_KEYS`) that forces `build_key()` back to MD5. Resolved once at import time. Set before `traffik.backends.base` is first imported to take effect.
