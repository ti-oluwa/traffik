...
class TestHTTPThrottleStat:
    async def test_http_throttle_stat_basic(self, backend: InMemoryBackend) -> None:
        ...
        routes = [Route("/api/data", endpoint, methods=["GET"])]
    ...
    async def test_http_throttle_stat_with_cost(self, backend: InMemoryBackend) -> None:
        ...
        routes = [Route("/api/expensive", endpoint, methods=["GET"])]
        app = Starlette(routes=routes)
    ...
    async def test_http_throttle_stat_at_limit(self, backend: InMemoryBackend) -> None:
        ...
        routes = [Route("/api/limited", endpoint, methods=["GET"])]
        app = Starlette(routes=routes)
    ...
    async def test_http_throttle_stat_different_keys(
        ...
        routes = [Route("/api/resource", endpoint, methods=["GET"])]
        app = Starlette(routes=routes)
        ...
        async with AsyncClient(
            ...
            response = await client.get("/api/resource?user=user1")
            assert response.json()["hits_remaining"] == 2  # Now 2 remaining
    ...
    async def test_http_throttle_stat_unlimited_rate(
        ...
        routes = [Route("/api/unlimited", endpoint, methods=["GET"])]
        app = Starlette(routes=routes)
    ...
    async def test_throttle_stat_fields(self, backend: InMemoryBackend) -> None:
        ...
        routes = [Route("/api/fields", endpoint, methods=["GET"])]
        app = Starlette(routes=routes)
