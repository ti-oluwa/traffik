...
- **Enhancements**:
  - Split the C extension in two: `traffik._hashing` (FNV-1a 32-bit and a new 64-bit variant) and `traffik._atomic` (the atomic byte-lock primitives used by `MultiProcessInMemoryBackend`, renamed from `_ext`). `_hashing` uses no compiler-specific intrinsics, so it now builds on every platform, Windows included. `_atomic` keeps the existing GCC/Clang-only restriction. Both extensions moved from `traffik.backends._cext`/`traffik.backends._ext` to the top-level `traffik._cext`/`traffik._atomic`, since hashing is no longer backend-specific.
  ...
  - `build_key()` (used internally by `ThrottleBackend.get_key(...)` when args/kwargs are passed) now hashes with FNV-1a 64-bit instead of MD5, producing a shorter 16-character digest. Falls back to MD5, if the compiled extension isn't available.
  ...
  - `InMemoryBackend.increment()`/`increment_with_ttl()` no longer round-trip counter values through `str()` internally. They now store the raw `int` and only stringify at the `get()`/`multi_get()` read boundary, preserving the existing `Optional[str]` return contract. `InMemoryBackend`'s shards switched from `OrderedDict` to plain `dict` as insertion order was never actually relied on (no LRU eviction, no `max_keys`).
  - Added `traffik._hashing.fnv_64bit_hash_hex()`, to get a 16-character hex string directly in C. `build_key()` now uses it instead of the previous md5 hashing, and is measured to be ~3x faster.
  - Added `traffik.config.get_legacy_md5_keys()`/`set_legacy_md5_keys()` (env var `TRAFFIK_LEGACY_MD5_KEYS`) that forces `build_key()` back to MD5. Resolved lazily on first use and cached, so it adds no per-call overhead - see the Bug Fixes entry below for why this isn't resolved at import time.
  - Renamed `_cext` to `_extensions` (and `_atomic.c` to `atomic.c`, for consistency with `hashing.c`). Source layout only, doesn't change any importable module name.
...
- **Bug Fixes**:
  ...
  - Fixed `traffik.config.set_legacy_md5_keys()` having no effect when called after `import traffik`. `build_key()`'s hash function was previously chosen once at *import* time (inside `traffik.backends.base`'s own module-level code, which always runs as a side effect of `import traffik`) - so by the time any application code got a chance to call `set_legacy_md5_keys()`, the choice had already been locked in and calling it did nothing. It's now resolved lazily on first call instead, and cached from then on: still zero per-call overhead after the first call, but now takes effect as long as `set_legacy_md5_keys()` runs before the first key is actually built, which in practice is before the first throttled request - well after any startup-time configuration code has run.
