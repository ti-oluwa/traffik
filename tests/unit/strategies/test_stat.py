...
class TestStrategyStat:
    async def test_fixed_window_stat(self, backend: InMemoryBackend):
        ...
        stat = await strategy.get_stat(key, rate, backend)
        assert round(stat.hits_remaining) == 7, "Should have 7 hits remaining"
        ...
        stat = await strategy.get_stat(key, rate, backend)
        assert round(stat.hits_remaining) == 0, "Should still have 0 hits remaining"
    ...
    async def test_sliding_window_log_stat(self, backend: InMemoryBackend):
        ...
        stat = await strategy.get_stat(key, rate, backend)
        assert round(stat.hits_remaining) == 3, "Should have 3 hits remaining"
    ...
    async def test_sliding_window_counter_stat(self, backend: InMemoryBackend):
        ...
        # Initial stat
        stat = await strategy.get_stat(key, rate, backend)
        assert round(stat.hits_remaining) == 8
    ...
    async def test_leaky_bucket_stat(self, backend: InMemoryBackend):
        ...
        # Initial stat
        stat = await strategy.get_stat(key, rate, backend)
        assert round(stat.hits_remaining) == 12, "Should start empty (can accept 12)"
    ...
    async def test_stat_with_cost(
        ...
        assert round(stat.hits_remaining) == 20

        # Make requests with different costs
        await strategy(key, rate, backend, cost=5)
        stat = await strategy.get_stat(key, rate, backend)  # type: ignore
        assert round(stat.hits_remaining) == 15, "Should account for cost=5"

        await strategy(key, rate, backend, cost=7)
        stat = await strategy.get_stat(key, rate, backend)  # type: ignore
        assert round(stat.hits_remaining) == 8, "Should account for cost=7"

        await strategy(key, rate, backend, cost=3)
        stat = await strategy.get_stat(key, rate, backend)  # type: ignore
        assert round(stat.hits_remaining) == 5, "Should account for cost=3"
    ...
    async def test_stat_after_window_reset(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
        ...
        assert 3 <= round(stat.hits_remaining) <= 5, "Should be reset to full limit"
        assert stat.wait_ms == 0.0, "Should have no wait time"
    ...
    async def test_stat_keys_isolation(
        ...
        stat1 = await strategy.get_stat("user:1", rate, backend)  # type: ignore
        assert round(stat1.hits_remaining) == 3

        # User 2 makes different requests
        await strategy("user:2", rate, backend, cost=2)
        stat2 = await strategy.get_stat("user:2", rate, backend)  # type: ignore
        assert round(stat2.hits_remaining) == 8

        # Stats should be independent
        assert stat1.hits_remaining != stat2.hits_remaining

    async def test_stat_wait_ms_accuracy(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
    ...
    async def test_stat_fields_present(
        self, backend: InMemoryBackend, strategy: ThrottleStrategy[HTTPConnection]
    ):
        ...
        assert isinstance(stat.wait_ms, (int, float)), (
            "'wait_ms' should be an integer or float"
        )
