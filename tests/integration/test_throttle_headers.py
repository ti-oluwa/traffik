...
class TestHTTPThrottleHeaders:
    async def test_static_string_headers(
        ...
        app = web_framework.build_app(
            http_routes=[HTTPRoute("/api/data", handler)],
    ...
    async def test_set_headers_applies_response_headers(
        self, inmemory_backend: InMemoryBackend, web_framework: ASGIFramework
    ) -> None:
        """The `set_headers` helper should resolve and apply headers to a response."""
        throttle = HTTPThrottle(
            uid=f"set-headers-{web_framework.name}",
            rate="5/s",
            identifier=default_client_identifier,
            headers={
                "X-RateLimit-Limit": Header.LIMIT(when="always"),
        ...
        async def handler(request: Request) -> JSONResponse:
            ...
            await throttle.set_headers(response, request)
            return response
