...
if sys.platform == "win32":
    # Skip WebSocket imports on Windows for now
    pass
...
def make_ws_app(throttle, backend) -> FastAPI:
    """
...
async def ws_scenario_below_limit(config: BenchmarkConfig, iteration: int = 1) -> ScenarioResult:
    """WS Below-Limit scenario."""
    backend = create_backend(config)
    try:
        if AsyncTestClient is None:
            raise ImportError("AsyncTestClient not available")
...
async def ws_scenario_over_limit(config: BenchmarkConfig, iteration: int = 1) -> ScenarioResult:
...
async def ws_scenario_burst(config: BenchmarkConfig, iteration: int = 1) -> ScenarioResult:
    """WS Burst scenario."""
    backend = create_backend(config)
    try:
        if AsyncTestClient is None:
...
async def ws_scenario_concurrent_connections(config: BenchmarkConfig, iteration: int = 1) -> ScenarioResult:
...
async def ws_scenario_window_boundary(config: BenchmarkConfig, iteration: int = 1) -> ScenarioResult:
