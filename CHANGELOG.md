...
**This release contains a breaking change to stored data**, read the note at the end of this section before upgrading.

- **Enhancements**:
  - Split the C extension in two: `traffik._hashing` (FNV-1a 32-bit and a new 64-bit variant) and `traffik._atomic` (the atomic byte-lock primitives used by `MultiProcessInMemoryBackend`, renamed from `_ext`). `_hashing` uses no compiler-specific intrinsics, so it now builds on every platform, Windows included; `_atomic` keeps the existing GCC/Clang-only restriction. Both extensions moved from `traffik.backends._cext`/`traffik.backends._ext` to the top-level `traffik._extensions`, since hashing is no longer backend-specific.
  - `cibuildwheel` no longer skips Windows or musllinux wheel builds. Windows previously had no compiled extension at all; it now gets `traffik._hashing` (`traffik._atomic` and `MultiProcessInMemoryBackend` remain POSIX-only).
  - `build_key()` (used internally by `ThrottleBackend.get_key(...)` when args/kwargs are passed) now hashes with FNV-1a 64-bit instead of MD5, producing a shorter 16-character digest.
  - Added `traffik._hashing.fnv_64bit_hash_hex()`: writes the 16-character hex digest directly in C. `build_key()` uses it and it is measured ~2x faster than the previous MD5 implementation.
  - Added `traffik.config.get_legacy_md5_keys()`/`set_legacy_md5_keys()` (env var `TRAFFIK_LEGACY_MD5_KEYS`) to force `build_key()` back to MD5. Resolved lazily on first call and cached from then on, so it adds no meaningful per-call overhead. See the Bug Fixes entry below for why it isn't resolved at import time.
  - `InMemoryBackend` and `MultiProcessInMemoryBackend` now reclaim expired entries via bounded random sampling (Redis-style active expiration) instead of a full scan of every live key/slot on each cleanup pass. Cost per cleanup cycle now scales with a small, configurable sample size, not with the number of live keys. The old `InMemoryBackend._cleanup()` held its shard lock for a full synchronous scan with no `await` inside, which could stall the entire event loop on a shard with many keys. Both backends gain a new `cleanup_sample_size` constructor param (default `20`) controlling how many keys/buckets are checked per shard, per round.
  - `InMemoryBackend.increment()`/`increment_with_ttl()` no longer round-trip counter values through `str()` internally; they store the raw `int` and only stringify at the `get()`/`multi_get()` read boundary, preserving the existing `Optional[str]` return contract. `InMemoryBackend`'s shards switched from `OrderedDict` to plain `dict` as insertion order was never actually relied on.
  - `traffik/__init__.py` (and `traffik.backends`/`traffik.throttles`) now use explicit imports and an explicit `__all__` instead of `import *`. A few names that were only ever reachable by accident (missing `__all__` in `traffik.backends.inmemory` and `traffik.throttles` let arbitrary non-underscore names leak through, including stdlib modules like `asyncio`/`random`/`functools`) are no longer importable from `traffik`; nothing that was intentionally public changed.
  - `AsyncRLock`, `FairAsyncRLock`, `NamedLockPool`, `NamedLockHandle`, and the new `AsyncLockAdapter` (which bridges a simple lock like `AsyncRLock` to the full `AsyncLock` protocol `ThrottleBackend.get_lock()` expects) are now public, importable from `traffik`. This makes the same lock primitives `InMemoryBackend` uses internally, available for anyone implementing a custom backend.
  ...
  - Simplified the rate string parser (`Rate.parse(...)`/`parse_rate_string`) to map each unit directly to its millisecond value instead of picking which of `Rate.__init__`'s four keyword arguments to fill. No behavior change.
...
**Note:** because `build_key()`'s hash algorithm changed, any already-persisted throttle state keyed via args/kwargs (e.g. `persistent=True` backends with long-lived counters) will appear as new keys after upgrading. Existing counters should be reset once, although they should auto expire and be then be discard. Ephemeral/expiring rate-limit state is unaffected in practice. Set `TRAFFIK_LEGACY_MD5_KEYS=1` after upgrading if you need to avoid this reset.
